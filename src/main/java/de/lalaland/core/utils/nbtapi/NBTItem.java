...
/**
 * NBT class to access vanilla/custom tags on ItemStacks. This class doesn't autosave to the
 * Itemstack, use getItem to get the changed ItemStack
 *
 * @author tr7zw
 */
public class NBTItem extends NBTCompound {
  ...
  /**
   * Helper method that converts {@link ItemStack} to {@link NBTContainer} with all it's data like
   * Material, Damage, Amount and Tags.
   *
   * @param item
  ...
  public static NBTContainer convertItemtoNBT(ItemStack item) {
    return NBTReflectionUtil
        .convertNMSItemtoNBTCompound(ReflectionMethod.ITEMSTACK_NMSCOPY.run(null, item));
  }

  /**
   * Helper method to do the inverse to "convertItemtoNBT". Creates an {@link ItemStack} using the
   * {@link NBTCompound}
   *
   * @param comp
