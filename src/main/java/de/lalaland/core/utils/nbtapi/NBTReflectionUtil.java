...
import de.lalaland.core.utils.nbtapi.nmsitemutils.GsonWrapper;
import de.lalaland.core.utils.nbtapi.nmsitemutils.nmsmappings.ClassWrapper;
import de.lalaland.core.utils.nbtapi.nmsitemutils.nmsmappings.ObjectCreator;
import de.lalaland.core.utils.nbtapi.nmsitemutils.nmsmappings.ReflectionMethod;
...
/**
 * Utility class for translating NBTApi calls to reflections into NMS code All methods are allowed
 * to throw {@link NbtApiException}
 *
 * @author tr7zw
 */
public class NBTReflectionUtil {
  ...
  public static Object getNMSEntity(Entity entity) {
    try {
      return ReflectionMethod.CRAFT_ENTITY_GET_HANDLE
          .run(ClassWrapper.CRAFT_ENTITY.getClazz().cast(entity));
    } catch (Exception e) {
  ...
  public static Object convertNBTCompoundtoNMSItem(NBTCompound nbtcompound) {
    try {
      if (MinecraftVersion.getVersion().getVersionId() >= MinecraftVersion.MC1_11_R1
          .getVersionId()) {
  ...
  public static NBTContainer convertNMSItemtoNBTCompound(Object nmsitem) {
    try {
      Object answer = ReflectionMethod.NMSITEM_SAVE
          .run(nmsitem, ObjectCreator.NMS_NBTTAGCOMPOUND.getInstance());
  ...
  public static Object getTileEntityNBTTagCompound(BlockState tile) {
    try {
      Object pos = ObjectCreator.NMS_BLOCKPOSITION
          .getInstance(tile.getX(), tile.getY(), tile.getZ());
    ...
    try {
      if ((boolean) ReflectionMethod.COMPOUND_HAS_KEY.run(compound, name)) {
        ...
        throw new NbtApiException(
            "Tried getting invalide compound '" + name + "' from '" + compound + "'!");
      }
    } catch (Exception e) {
