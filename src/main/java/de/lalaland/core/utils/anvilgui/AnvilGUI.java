...
public class AnvilGUI {
  ...
  private static AnvilImplementation WRAPPER;
  ...
  /**
   * Create an AnvilGUI and open it for the player. Left slot has a model data of 2000 in 1.14+
   * builds. (PAPER) The right slot has a model data of 2002 in 1.14+ builds. (COAL)
   *
   * @param holder     The {@link Player} to open the inventory for
   * @param insert     What to have the text already set to
   * @param biFunction A {@link BiFunction} that is called when the player clicks the {@link
   *                   Slot#OUTPUT} slot
  ...
  public AnvilGUI(Player holder, String insert, BiFunction<Player, String, String> biFunction) {
    ...
    final ItemStack paper = new ItemBuilder(Material.PAPER).modelData(2000).build();
  ...
  public static class Slot {
    ...
    /**
     * The slot on the far left, where the first input is inserted. An {@link ItemStack} is always
     * inserted here to be renamed
     */
    ...
    /**
     * The output slot, where an item is put when two items are combined from {@link #INPUT_LEFT}
     * and {@link #INPUT_RIGHT} or {@link #INPUT_LEFT} is renamed
     */
